...
export function Nests() {
  ...
  const handleCreateNest = async () => {
    ...
    const input: { name: string; description?: string } = { name: newNestName.trim() }
    if (newNestDescription.trim()) {
      input.description = newNestDescription.trim()
    }
      ...
      {nests.length === 0 ? (
        ...
        <div style={styles.list}>
          {nests.map((membership: any) => {
            ...
            const memberCount = nest.members?.length ?? 0
